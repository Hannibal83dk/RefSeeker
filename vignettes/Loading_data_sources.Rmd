...
## Loading data from excel or ODS files

The procedure for loading excel or ODS file are exactly the same. The example provided below for excel files therefore also apply for ods. For sake of coherence, the rs_loadodsdata() function can replace the rs_loadexceldata(). However, this is actually not a requirement since the two functions are synonymous aliases for the same function.
