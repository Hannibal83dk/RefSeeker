...
#'
#' dct <- rsorderbystability(dct)
#' dct
#'
rsorderbystability <- function(df){
  df <- df[order(df[,2]),]
  row.names(df) <- NULL
  return(df)
}
...
#'
#' @param stabilitytable A stability table, normally generated by an rs algorithm
...
rsaddstabilityrank <- function(stabilitytable, stabilitycol) {


  stabilitytable <- rsorderbystability(stabilitytable)
